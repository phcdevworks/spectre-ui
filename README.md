...
Boolean options are caller-owned: omission adds no corresponding modifier
class, `true` adds it, and `false` omits it. Component packages define their
own property defaults and pass resolved boolean values explicitly. Non-boolean
...
| Badge       | `getBadgeClasses`       | `primary` `secondary` `success` `warning` `danger` `neutral` `info` `ghost` `accent` `cta` `inverse`           | `sm` `md` `lg`                       | `interactive` `disabled` `loading` `fullWidth`                                                                                        |
| Card        | `getCardClasses`        | `elevated` `flat` `outline` `ghost`                                                                            | `padded`: `sm` `md` `lg`             | `interactive` `padded` (also accepts a size) `fullHeight` `disabled` `loading`                                                       |
...
| Spinner     | `getSpinnerClasses`     | —                                                                                                              | `sm` `md` `lg`                       | `disabled` `loading`                                                                                                                   |
...
drift, or when CSS contract coverage no longer matches the declared surface.
...
- generated JS, TypeScript declarations, CSS bundles, README docs, and
  `ui-contract.manifest.json` are expected to stay aligned
...
- `@phcdevworks/spectre-ui` turns those tokens into reusable CSS and type-safe
  class recipes
...
README validation → token drift check → build → CSS contract → tests. All steps
must pass.
